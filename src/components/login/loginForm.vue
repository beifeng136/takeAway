...
export default {
  ...
  setup() {
    ...
    const sendData = async (value) => {
      const { data: res } = await axios.post("/user/login", value);
      if (res === "") {
        state.notifyContent = "用户名或密码错误";
        notifySwitch();
      } else {
        state.notifyContent = "登录成功";
        notifySwitch(2000, () => {
          localSet("userInformation", res);
          router.push("/home");
        });
      }
    };
    // 指定callback 回调函数 如果需要页面的跳转可以传入回调函数
